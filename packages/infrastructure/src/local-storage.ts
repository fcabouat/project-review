 ...
 * of the same origin, never on the one that wrote. Narrowed to the two keys
 * that govern this tab's saving — the saved DOCUMENT and the PREFERENCE that
 * says whether to save at all, because switching the save off in one tab is
 * news every other tab must hear rather than discover at its next write. A
 * `null` key means the whole storage was cleared, which concerns both. Nothing
 * is read here: the caller compares what it finds and decides.
 *
...
export const watchStored: WatchStored = (onChange): Cancel => {
  const listener = (event: StorageEvent): void => {
    if (event.key === null || event.key === STATE_KEY || event.key === PREF_KEY) onChange()
  }
  try {
