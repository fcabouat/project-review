 ...
 * browser's cross-tab signal to the two keys that matter, and
...
describe('watchStored', () => {
  ...
  it('fires on the document, on the save preference, and on a storage cleared wholesale', () => {
    ...
    try {
      ...
      host.fire(STATE_KEY)
      // The PREFERENCE is shared state as much as the document: a tab that
      // switches the local save off is asking this browser to keep nothing,
      // and the other tabs must hear it now rather than at their next write.
      host.fire(PREF_KEY)
      // `null` is how the browser announces a `clear()`: it concerns both,
      // so the caller must hear about it.
      host.fire(null)
      expect(calls).toBe(3)
