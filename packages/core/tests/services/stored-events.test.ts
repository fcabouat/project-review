...
describe('decodeEvent — every legitimate event survives the round trip', () => {
  it('accepts each of the exhaustive samples, serialised and read back', () => {
    // The table names every variant at the type level; apply-invert.test.ts
    // separately checks that each variant has at least one sample.
    for (const sample of everySample) {
      const decoded = decodeEvent(roundTrip(sample))
      expect(decoded, `refused a legitimate ${sample.type}: ${JSON.stringify(sample)}`).toEqual(
        roundTrip(sample),
      )
    }
  })
...
describe('decodeEvent — exact keys, exact types', () => {
  it('refuses unusable ids and duplicate font slots in persisted events too', () => {
    ...
    expect(
      decodeEvent({ type: 'SettingChanged', setting: 'fontFaces', after: [face, { ...face }] }),
    ).toBeUndefined()
  })
