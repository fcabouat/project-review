...
import { HISTORY_LIMIT, emptyHistory, type History } from '../../src/events/history'
...
describe('the log rides inside the envelope', () => {
  ...
  it('a successful write names the log it stored — the very object, when it fits', () => {
    // The caller remembers what the storage holds so that the closing save
    // can decide to write NOTHING; a fresh array on every write would make
    // that comparison differ every time, and every page close would announce
    // a change nobody made to every other tab.
    const storage = createMemoryStorage()
    const log = {
      past: [{ type: 'ReviewFieldChanged', field: 'title', before: 'a', after: 'b' }],
      future: [],
    } as const satisfies History
    const outcome = writeState(storage, null, testPortfolio(), log)

    expect(outcome.outcome).toBe('written')
    if (outcome.outcome !== 'written') return
    expect(outcome.history).toBe(log)
    expect(restored(storage)!.history).toEqual(log)
  })

  it('and names the NARROWED log when a cap moved it', () => {
    const storage = createMemoryStorage()
    const event = (i: number): DomainEvent => ({
      type: 'ReviewFieldChanged',
      field: 'title',
    ...
    const past = Array.from({ length: HISTORY_LIMIT + 2 }, (_, i) => event(i))
    const outcome = writeState(storage, null, testPortfolio(), { past, future: [] })

    expect(outcome.outcome).toBe('written')
    if (outcome.outcome !== 'written') return
    // What went in, not what was offered: a caller that remembered the
    // offered log would believe the storage holds two events it never took.
    expect(outcome.history.past).toHaveLength(HISTORY_LIMIT)
    expect(outcome.history).toEqual(restored(storage)!.history)
  })

  it('and says the log was dropped whole when only the document fitted', () => {
    // The trade the ceiling makes: losing undo steps is not losing the
    // document. The caller is TOLD, so it does not go on believing the
    // storage holds a trail it refused.
    const storage = createMemoryStorage()
    const heavy = (i: number): DomainEvent => ({
      type: 'ReviewFieldChanged',
      field: 'title',
      before: 'x'.repeat(20_000),
      after: `${i}`,
    })
    const past = Array.from({ length: 200 }, (_, i) => heavy(i))
    const outcome = writeState(storage, null, testPortfolio(), { past, future: [] })

    expect(outcome.outcome).toBe('written')
    if (outcome.outcome !== 'written') return
    expect(outcome.history).toStrictEqual(emptyHistory)
    expect(restored(storage)!.portfolio).toEqual(testPortfolio())
    expect(restored(storage)!.history).toStrictEqual(emptyHistory)
  })
})
