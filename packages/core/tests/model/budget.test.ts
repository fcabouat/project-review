...
describe('the byte ceiling, weighed on EVERY command — not on a list of the growing ones', () => {
  ...
  it('refuses the oversize payload of every variant that can carry one', () => {
    for (const [type, oversize] of Object.entries(OVERSIZE) as [
      ...
      // for any variant, the whole-document one included (its shape rule is
      // `validPortfolioShape`, which measures nothing) …
      expect(honorsContract(p, oversize), type).toBe(true)
      // … and the budget is what refuses, on the state the event would leave.
      expect(decide(p, oversize), type).toBeUndefined()
      // And the refusal SAYS which of the two it was — the import dialog
      // shows that sentence.
      expect(verdict(p, oversize), type).toEqual({ ok: false, refusal: 'overBudget' })
    }
  })
