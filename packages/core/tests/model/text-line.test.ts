...
import { MAX_LINE_CHUNKS, parseLine } from '../../src/model/text-line'
...
describe('TextLine micro-format', () => {
  ...
  it('bounds markup expansion without discarding text or suffix', () => {
    const atLimit = '**a**'.repeat((MAX_LINE_CHUNKS - 1) / 2)
    expect(parseLine(atLimit).segments.every((segment) => segment.bold)).toBe(true)
    const aboveLimit = atLimit + '**b**'
    expect(parseLine(aboveLimit + ' — 20 k€')).toEqual({
      segments: [{ bold: false, text: aboveLimit }],
      suffix: '20 k€',
    })
  })
})
