...
/** A whole number — what every index and every `from`/`to` must be. The
 * splice helpers clamp a number into range, but `NaN` and `"3"` walk straight
 * past that clamp and silently reorder a collection. */
const isWhole = (x: unknown): boolean => typeof x === 'number' && Number.isInteger(x)
