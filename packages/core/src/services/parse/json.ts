...
import { isCategoryReference } from '../../values/ids'
...
export function idStr(x: unknown, seen: Set<string>, path: string, errors: Errors): string {
  const s = idReference(x, path, errors) ?? ''
...
/** A Unicode reference, including the empty unassigned value. Invalid input
 * remains untouched so the enclosing parse can report every violation. */
export function idReference(x: unknown, path: string, errors: Errors): string | undefined {
  const s = str(x, path, errors)
  if (s !== undefined && !isCategoryReference(s)) fail(errors, path, 'invalidId')
  return s
}
