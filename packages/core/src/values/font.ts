...
/** Identity of one embedded variant; file content does not make a new slot. */
export const fontFaceSlot = (face: {
  readonly family: string
  readonly weight: string
  readonly style: string
}): string => `${face.family}/${face.weight}/${face.style}`
 ...
 * 100–900, or an ASCENDING "min max" pair of such integers (a variable face
 * covering a range).
 */
export const isFontWeight = (weight: string): boolean => {
  ...
  return min >= 100 && max <= 900 && min <= max
}
...
/** {@link FONT_FACE_MAX_CHARS} said in DECLARED BYTES — what a picked file may
...
export const FONT_FACE_MAX_BYTES = maxDeclaredBytes(FONT_FACE_MAX_CHARS)
