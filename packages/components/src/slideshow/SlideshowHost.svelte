...
<script lang="ts">
  /**
   * Full-screen presentation of one immutable portfolio snapshot. Its tree,
   * templates, print view and export all read that snapshot. Reveal's engine
   * and base CSS are loaded at mount; the owned stylesheet leaves on teardown.
   * The 1280 × 720 canvas is scaled, not reflowed. Slideshow imports no editor.
   */
  ...
  const snapshot = untrack(() => portfolio)
   ...
   */
  const groups = deckTree(snapshot)
  ...
  /** Failed engine imports or initialization offer an explicit retry. */
  ...
  async function save(): Promise<void> {
    ...
    saveState = 'saving'
    try {
      await exportStandalone(slidesEl, snapshot)
      saveState = 'done'
      setTimeout(() => {
