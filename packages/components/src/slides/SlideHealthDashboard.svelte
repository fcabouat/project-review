<script lang="ts">
  ...
  const MAX_ROWS = 8
  /** The health-column inset is a summary, not an exhaustive milestone table. */
  const MAX_OVERDUE = 5
      ...
      <div class="callout mt-(--slide-step) rounded-lg px-3.5 py-3">
        ...
        {#each overdue.slice(0, MAX_OVERDUE) as item (`${item.project.id}:${item.index}`)}
          <p class="mt-[5px] flex gap-1 text-[13px] leading-[1.35] print:text-[12.5px]">
            ...
            >
            <span class="dim flex-none"
              >— {item.milestone.display ?? formatShortDate(item.milestone.date)}</span
            >
          </p>
        {/each}
        {#if overdue.length > MAX_OVERDUE}
          <p class="dim mt-[5px] text-[13px] leading-[1.35] print:text-[12.5px]">
            {t('d2.more', language, { n: overdue.length - MAX_OVERDUE })}
          </p>
        {/if}
      </div>
    {/if}
