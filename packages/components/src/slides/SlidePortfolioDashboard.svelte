<script lang="ts">
  ...
  const rows = $derived(
    categoryBars(portfolio).map((bar) => {
      const category = categoryOfGroup(portfolio, bar.group)
      return {
        bar,
        ...
        count: projectsOfGroup(portfolio, bar.group).filter(isTracked).length,
      }
    }),
  )
...
>
  ...
  <div class="mt-(--slide-step) flex min-h-0 flex-1 flex-col justify-between">
    {#each rows as row (groupKey(row.bar.group))}
      <div
