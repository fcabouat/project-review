...
export function categoryName(category: Category | UnsortedCategory, language: Language): string {
  // Grey belongs only to the display sentinel; real category ids are unrestricted names.
  return category.color === 'grey' ? unsortedCategoryName(language) : category.name
}
