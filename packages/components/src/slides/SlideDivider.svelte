<script lang="ts">
  ...
  interface Props {
    ...
    /** The group this divider opens — a category, or the orphan residue. */
    readonly group: GroupRef
    /** Rank of the divider in the deck, shown as the monumental numeral. */
    readonly number: number
  }

  let { portfolio, group, number }: Props = $props()

  /** Beyond 10 the column would run past the floor: the list gets truncated. */
  const MAX_ITEMS = 10

  const language = $derived(portfolio.settings.language)
  const category = $derived(categoryOfGroup(portfolio, group))
  const color = $derived(catColor(category.color))

  // The group's own selection, exactly the one `deck()` walked to emit this
  // divider: for the orphan group that is every project whose category id
  // resolves to nothing, whatever that id says.
  const projects = $derived(projectsOfGroup(portfolio, group))
  const tracked = $derived(projects.filter(isTracked))
  // The sheet figure counts the sheets the DECK emits — over all the group's
  // projects, so an archived one kept visible by `sheet: 'always'` is counted:
  // a tracked-only count announces one fewer than the section holds.
  const sheets = $derived(projects.filter(showsSheet).length)
