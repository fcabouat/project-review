<script lang="ts">
   ...
   * `ReplacePortfolio` command after confirmation. Appearance profiles can
   * only be mixed, never replace content. Invalid input is refused whole
   * with a localized, path-by-path report.
   *
...
<Dialog.Root open onOpenChange={(o) => o || close()}>
  <Dialog.Content
    class="top-11 max-h-[calc(100dvh-60px)] w-[640px] max-w-[calc(100%-32px)] translate-y-0 gap-0 overflow-x-hidden overflow-y-auto p-0 sm:max-w-[640px]
  ...
  >
    ...
    <Tabs.Root value={active} onValueChange={(v) => (active = v as 'export' | 'import')}>
      ...
      >
        ...
        >
        <Tabs.Trigger value="import" class={triggerClass} disabled={readOnly}
      ...
      <Tabs.Content value="import">
        {#if !readOnly}<ImportPanel {portfolio} {dispatch} {close} {prefill} {prefillMode} />{/if}
      </Tabs.Content>
    </Tabs.Root>
