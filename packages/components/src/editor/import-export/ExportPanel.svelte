<script lang="ts">
  ...
  import { groupKey, orphanProjects, type GroupRef } from '@project-review/core/projections'
  import { appearanceProfile } from '@project-review/core/services/portfolio-import'
  ...
  const groups = $derived.by((): readonly ExportGroup[] => {
    const byCategory = portfolio.categories.map((category): ExportGroup => ({
      ref: { kind: 'category', id: category.id },
      name: category.name,
    ...
    return [
      ...
      ...(orphans.length > 0
        ? [
            {
              ref: { kind: 'orphans' as const },
              name: te('category.unsorted', language),
              projects: orphans,
            },
          ]
        : []),
  ...
  const exportSource = $derived(
    profileOnly
      ? JSON.stringify(appearanceProfile(portfolio), null, 2)
      : allSelected
        ? serialised
        : serializePortfolio(partialPortfolio(portfolio, selectedIds)),
  )
  const exportName = $derived(
    profileOnly
      ? 'project-review-appearance.json'
      : portfolioFileName(portfolio.review.reviewDate, !allSelected),
  )
...
<div class="p-5">
  <label class="mb-3 flex items-start gap-2 text-sm">
    <Checkbox bind:checked={profileOnly} />
    <span>{te('editor.io.exportProfile', language)}</span>
  </label>
  {#if profileOnly}
    <p class="text-muted-foreground mb-3 text-xs">{te('editor.io.exportProfileHint', language)}</p>
