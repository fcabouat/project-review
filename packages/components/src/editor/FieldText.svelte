...
<script lang="ts">
  ...
  const saveState = $derived(
    draftKey !== undefined
      ? drafts?.getSaveState
        ? drafts.getSaveState(draftKey, unrecorded)
        : unrecorded
          ? 'pending'
          : undefined
      : unrecorded
  ...
  const showSaveState = $derived(touched && saveState !== undefined)
  const saveLabel = $derived(
    saveState === undefined ? '' : te(`editor.save.${saveState}`, language),
  )
...
<label class={compact ? 'flex min-w-0 flex-col' : 'mb-3.5 flex flex-col last:mb-0'}>
  ...
  <div class="relative">
    {#if rows}
      <Textarea
        class={monospace
          ? 'text-(--txt2) field-sizing-fixed bg-[#fafafa] pr-8 font-mono text-xs leading-[1.55] dark:bg-white/5'
          : 'field-sizing-fixed pr-8 text-sm leading-[1.4]'}
        {rows}
        {placeholder}
        {readonly}
