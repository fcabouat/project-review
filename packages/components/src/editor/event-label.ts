...
function formatValue(field: string, value: unknown, language: Language): string {
  ...
  switch (field) {
    case 'stage':
      return te(`stage.${String(value)}`, language)
    case 'health':
      return te(`level.${String(value)}`, language)
    case 'sheet':
      return te(`editor.sheetMode.${String(value)}`, language)
    case 'palette':
      return te(`editor.palette.${String(value)}`, language)
    case 'style':
      return te(`editor.style.${String(value)}`, language)
    case 'logo':
...
export function eventLabel(event: DomainEvent, language: Language, nameOf?: NameLookup): string {
  ...
  switch (event.type) {
    ...
    case 'CategoryDeleted':
      return te(byType, language, { name: event.category.name })

    case 'CategoryMoved':
      // Routed by id; the event carries no name (nothing derived is stored).
      return te(byType, language, { name: named('category', event.id) })
    ...
    case 'ProjectCreated':
    case 'ProjectDeleted':
      return te(byType, language, { id: event.project.id })

    case 'ProjectMoved':
      return te(byType, language, { id: event.id })

    case 'ProjectRenumbered':
      return te(byType, language, { before: event.oldId, after: event.newId })
    ...
    case 'PortfolioReplaced':
      return te(byType, language, { n: event.after.projects.length })
    ...
    case 'ProjectsMerged': {
      ...
      return te(byType, language, { n: report.replaced, m: report.added })
    }
  }
