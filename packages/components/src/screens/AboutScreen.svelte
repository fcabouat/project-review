...
<div class="mx-auto flex max-w-[760px] flex-col gap-4">
  ...
  <section class="bg-background border-border rounded-lg border p-4">
    ...
    <p class="text-muted-foreground mb-2 text-[12.5px] leading-[1.55]">
      {te('editor.about.licenseTextsLead', language)}<a
        class="underline"
        href="./THIRD-PARTY-LICENSES.txt"
        target="_blank"
        rel="noreferrer noopener">THIRD-PARTY-LICENSES.txt</a
      >{te('editor.about.licenseTextsTrail', language)}
    </p>
    <!-- The same sentence the README carries and the font-embedding form
         ...
         their own document. -->
    <p
      class="text-muted-foreground text-[12.5px] leading-[1.55]"
      lang={language === 'fr' ? 'en' : undefined}
    >
      {IMPORTED_CONTENT_NOTICE}
    </p>
  </section>
  ...
  <section class="bg-background border-border rounded-lg border p-4">
    ...
    <ul class="m-0 flex list-none flex-col gap-2.5 p-0">
      {#each THIRD_PARTY_NOTICES as notice (notice.name)}
        <li
          class="border-border wrap-anywhere rounded-md border px-3.5 py-2.5"
          lang={language === 'fr' ? 'en' : undefined}
        >
