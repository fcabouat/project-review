<script lang="ts">
  ...
  import { maxDeclaredBytes } from '@project-review/core/values/data-uri'
  import { FONT_FACE_MAX_BYTES, fontFaceSlot, isFontFamily } from '@project-review/core/values/font'
  import { LOGO_MAX_BYTES } from '@project-review/core/values/logo'
  ...
  /** Refusal line of one file too big for a face, worded on the cap itself. */
  const faceTooBig = (name: string): string =>
    te('editor.settings.fontFaceTooBig', language, {
      name,
      max: Math.round(FONT_FACE_MAX_BYTES / 1024),
    })

  /** Refusal line of a selection too big as a whole, same arithmetic. */
  const totalTooBig = (): string =>
    te('editor.settings.fontsTotalTooBig', language, {
      max: Math.round(maxDeclaredBytes(FONT_FACES_TOTAL_MAX_CHARS) / 1024),
    })
   ...
   *
   * THE DECLARED SIZES ARE JUDGED FIRST, BEFORE ONE FILE IS READ. Reading a
   * font is a full base64 allocation, and a 40 MB typeface answered the cap
   * only after paying for it — the caps are stated in characters, so the
   * bound on a declared size derives from them (`maxDeclaredBytes`, the same
   * arithmetic as the JSON import's own pre-check). Both ceilings are
   * pre-checked, the per-face one and the total, and both are re-checked on
   * the strings that actually come back: the bound says what CANNOT fit, the
   * final check says what does.
   */
  async function embed(files: FileList | null): Promise<void> {
    ...
    for (const file of woff2) {
      if (file.size > FONT_FACE_MAX_BYTES) refused.push(faceTooBig(file.name))
      else candidates.push(file)
    }
    // The total, on the same bound one ceiling up: what is left of it once the
    // faces this pick does NOT replace are counted. Over that, no reading can
    // make the collection fit, so the pick is refused whole.
    const replaced = new Set(candidates.map((file) => fontFaceSlot(faceFromFileName(file.name)!)))
    const kept = fontFaces
      .filter((face) => !replaced.has(fontFaceSlot(face)))
      .reduce((sum, face) => sum + face.dataUri.length, 0)
    const declared = candidates.reduce((sum, file) => sum + file.size, 0)
    if (declared > maxDeclaredBytes(FONT_FACES_TOTAL_MAX_CHARS - kept)) {
      embedErrors = [...refused, totalTooBig()]
      return
    }
    ...
    const put = (face: EmbeddedFontFace): void => {
      const i = faces.findIndex((f) => fontFaceSlot(f) === fontFaceSlot(face))
      if (i >= 0) faces[i] = face
    ...
    for (const file of candidates) {
      ...
      if (dataUri.length > FONT_FACE_MAX_CHARS) {
        refused.push(faceTooBig(file.name))
        continue
      }
   ...
   * on the declared size first (an image over `LOGO_MAX_BYTES` cannot encode
   * under the cap, so it is refused unread), then on the URI that comes back. */
  ...
  <div class="mb-4 flex flex-col gap-[7px]">
    ...
    {#if fontFaces.length > 0}
      <ul class="flex flex-col gap-1">
        {#each fontFaces as face (fontFaceSlot(face))}
          <li
