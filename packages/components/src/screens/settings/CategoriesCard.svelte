<script lang="ts">
  ...
  /** A real category is always a `category` group — the orphan residue has no
   * card here, and could not be edited if it had one. */
  const groupOf = (category: Category): GroupRef => ({ kind: 'category', id: category.id })
  ...
  /** The dividers the deck ACTUALLY emits, number by group key — read off
   * `deck()` itself rather than re-derived here, so the divider preview can
   * never disagree with the slideshow (one derivation, law 3). Keyed by
   * `groupKey`, which is why a category named `unsorted` reads its own number
   * and not the orphan group's. */
  const dividerNumbers = $derived(
    new Map(
      deck(portfolio).flatMap((s) =>
        s.type === 'divider' ? [[groupKey(s.group), s.number] as const] : [],
      ),
    ),
  )

  function hasDivider(category: Category): boolean {
    return dividerNumbers.has(groupKey(groupOf(category)))
  }

  function dividerNumber(category: Category): number {
    return dividerNumbers.get(groupKey(groupOf(category))) ?? 0
  }
