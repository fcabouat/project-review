...
<script lang="ts">
  ...
  const waiting = createPersistenceMock({ revision: 7, phase: 'pending' })
  const switchedOff = createPersistenceMock({ revision: 7, phase: 'off' })
...
<!-- A field is holding what someone typed and the document has not recorded
     it yet: one field ↔ one event, at blur (`editor/drafts.ts`). The state on
     screen really is saved, and saying « saved » would still be false about
     the person's work — so the strip says what is actually true, quietly: it
     carries no action, because leaving the field records it and a closing
     page commits it. -->
<Story name="Typing not recorded yet" asChild>
  <Shell
    portfolio={store.present}
    past={store.past}
    future={store.future}
...
<!-- Someone switched the local save off in this browser — this tab or another
     one, since the preference is shared state. The save stops there and then
     and it is SAID, with the same offer as a refused write plus where to turn
     it back on: a privacy choice is reversed by a person, never by a save. -->
<Story name="Local save switched off elsewhere" asChild>
  <Shell
    portfolio={store.present}
    past={store.past}
    future={store.future}
