...
describe('raw draft checkpoints', () => {
  ...
  it('exposes truthful per-field save state across saved, draft, and unsafe phases', () => {
    const { storage, store, wiring, type } = setup()
    expect(wiring.drafts.getSaveState?.('review.title', false)).toBe('saved')

    type('unfinished')
    expect(wiring.drafts.getSaveState?.('review.title', true)).toBe('pending')
    vi.advanceTimersByTime(DRAFT_IDLE_MS)
    expect(wiring.control.save?.phase).toBe('draftSaved')
    expect(wiring.drafts.getSaveState?.('review.title', true)).toBe('draftSaved')
    ...
    expect(wiring.drafts.getSaveState?.('review.title', true)).toBeUndefined()
    expect(wiring.drafts.getSaveState?.('review.title', false)).toBeUndefined()
