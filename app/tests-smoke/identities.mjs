...
/** Lead lines must not push the summary into the slide footer, including on paper. */
      ...
      for (const media of ['screen', 'print']) {
        ...
        const fits = await table.evaluate(
          (element) =>
            ...
            element.closest('.slide').querySelector('.slide-foot').getBoundingClientRect().top - 4,
        )
        assert.ok(fits, `${style}/${media}: summary must leave room for the footer`)
      }
      console.log(`  ok — ${style}: summary leads fit on screen and paper`)
    } finally {
      await context.close()
    }
  }
