...
async function editorPass(browser, base, mode) {
  ...
  await context.addInitScript(
    ({ scheme, portfolio }) => {
      localStorage.setItem('project-review/scheme', scheme)
      localStorage.setItem(
        'project-review/state',
        JSON.stringify({ format: 1, revision: 1, portfolio, history: { past: [], future: [] } }),
      )
    },
    { scheme: mode, portfolio: sample },
  )
  const page = await context.newPage()
  const app = `${base}/dist/index.html`
  ...
  for (const [label, surface] of [
    ...
    await scan(page, surface, mode)
    if (label === 'Importer…') {
      await page.getByRole('textbox', { name: 'JSON à importer' }).fill(JSON.stringify(sample))
      await page.getByRole('radio', { name: /Panacher/ }).check()
      await scan(page, 'selective import preview', mode)
    } else {
      await page.getByRole('checkbox', { name: /profil d’apparence/ }).check()
      await scan(page, 'appearance profile export', mode)
    }
  ...
  await page.goto(`${app}?sample&lang=fr`)
  await page.getByTestId('demo-marker').waitFor()
  await scan(page, 'read-only demo', mode)
