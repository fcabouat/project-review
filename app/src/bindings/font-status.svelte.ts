 ...
 *  - covered by embedded faces → `embedded` only after the browser confirms
 *    they load: a present data URI can still contain invalid font bytes;
    ...
    watch(family: string, embedded: readonly string[] = []) {
      ...
      if (embedded.includes(clean)) {
        ...
        void probe(clean).then((verdict) => {
          ...
          status = verdict === 'served' ? 'embedded' : verdict === 'missing' ? 'missing' : 'unknown'
        })
        return
      }
