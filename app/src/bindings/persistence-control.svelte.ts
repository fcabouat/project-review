...
import type { FieldSaveState } from '@project-review/components/editor/drafts'
...
export function createPersistenceControl(
  ...
  // Keep the field UI on the same reactive persistence signal as the global
  // control. Hostile storage phases deliberately return no field checkmark.
  const draftsForComponents = Object.create(drafts) as DraftRegistry
  Object.defineProperty(draftsForComponents, 'getSaveState', {
    value: (_key: string, dirty: boolean): FieldSaveState | undefined => {
      void revision
      const phase = session.control.save?.phase
      if (phase === 'error' || phase === 'conflict' || phase === 'off' || phase === 'unavailable')
        return undefined
      if (dirty) return phase === 'draftSaved' ? 'draftSaved' : 'pending'
      return phase === 'saved' ? 'saved' : undefined
    },
  })
