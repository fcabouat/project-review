 ...
 *
 * 1. THE SAMPLE SET, loaded through the editor's ordinary import dialog from
 *    `dist/sample-portfolio.{en,fr}.json`. The public `?sample` demo is read-only.
 *    All but the two embedded-font pictures use that set, untouched:
 ...
 * two the merge appends). Only projects are selected in this staging;
 * categories, review, settings and free slides remain unchanged.
 * The figures are therefore fixed by construction: 1 replaced, 2 added.
 */
const contribution = (sample) => ({
  ...sample,
  projects: [
    { ...sample.projects[0], name: sample.projects[0].name + ' (v2)' },
...
async function importMerge(page, language, sample) {
  ...
  const categories = page.locator('details').filter({
    has: page.locator('summary', { hasText: language === 'fr' ? 'Catégories —' : 'Categories —' }),
  })
  await categories.locator('summary').click()
  const projects = page.locator('details').filter({
    has: page.locator('summary', { hasText: language === 'fr' ? 'Projets —' : 'Projects —' }),
  })
  await projects.locator('summary').click()
  await projects
    .getByRole('checkbox', {
      name: language === 'fr' ? 'Tout sélectionner' : 'Select all',
      exact: true,
    })
    .check()
  ...
  const preview = page.getByRole('region', {
    name: language === 'fr' ? 'Récapitulatif avant validation' : 'Preview before applying',
  })
  const counts = await preview.innerText()
  if (
    !counts.includes(language === 'fr' ? '2 ajout(s) · 1 remplacement(s)' : '2 added · 1 replaced')
  )
    fail(`${language}: incorrect mix preview: ${counts}`)
  // Show the selected projects and counted preview, not a page of raw JSON.
...
/**
 * The seven pictures of the shipped sample set, imported in one editor context.
 * Nothing is seeded into storage by hand: these pictures show the real import
 * result and stay editable, unlike the public read-only demo.
 */
async function samplePictures(browser, base, language, sample) {
  ...
  const app = `${base}/${APP}?lang=${language}`
  await page.goto(app)
  await page.getByRole('button', { name: LABEL.importNav[language] }).click()
  ...
  await page
    .getByRole('button', {
      name:
        language === 'fr'
          ? 'Remplacer le portefeuille (annulable)'
          : 'Replace the portfolio (undoable)',
    })
    .click()
  ...
  await open('#/projects')
  // Fail if import did not load the sample, rather than capturing a blank editor.
  if (!(await page.getByText(LABEL.projectsTotal[language]).first().isVisible())) {
    fail(`${language}: import did not load the ${SAMPLE_PROJECTS}-project sample set`)
  }
...
async function main() {
  ...
  // Screenshots need only the distributable, never the repository or its metadata.
