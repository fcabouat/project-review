...
const OUT = '_site'
const page = (title: string, body: string, lang: string, depth = 0) => `<!doctype html>
...
writeFileSync(
  ...
  page('project-review — overview', md('docs/overview.md'), 'en'),
)
writeFileSync(
  join(OUT, 'guide/en.html'),
  page('project-review — user guide', md('docs/user-guide.md', 1), 'en', 1),
)
writeFileSync(
  join(OUT, 'guide/fr.html'),
  page('project-review — guide utilisateur', md('docs/user-guide.fr.md', 1), 'fr', 1),
)
